...
# ⚙️ কাস্টম প্রতিস্থাপন টেবিল জেনারেট করা
...
def fetch_firebase_remote_config():
  ...
  if not api_key or not project_id or not app_id:
    print(
        "❌ ভুল: ফায়ারবেসের প্রয়োজনীয় সিক্রেট ভ্যালুগুলো"
        " এনভায়রনমেন্টে সেট করা নেই!"
    )
    return None
  ...
  try:
    ...
    if response.status_code == 200:
      ...
      return config_data.get("entries", {}).get("api_url", "")
    else:
      print(
          "❌ ফায়ারবেস থেকে ডাটা পাওয়া যায়নি। স্ট্যাটাস:"
...
def fetch_and_decrypt_link(base_url, link_path):
  headers = {
      "User-Agent": (
          "Mozilla/5.0 (Windows NT 10.0; Win64; x64) GitHubActions/1.0"
      )
  }
    ...
    if res.status_code == 200:
      ...
      if decrypted_content:
        try:
          ...
          return [
              l.strip() for l in decrypted_content.split("\n") if l.strip()
          ]
    return []
  except Exception:
    return []
...
def run():
  ...
  if not base_url_from_firebase:
    print(
        "❌ মেমোরিতে কোনো BASE_URL পাওয়া যায়নি। স্ক্রিপ্ট বন্ধ করা হচ্ছে।"
    )
    return
  ...
  try:
    ...
    print("🔓 মূল ইভент ডাটা ডিক্রিপ্ট করা হচ্ছে...")
    ...
    if decrypted_json_str:
      ...
      if final_events:
        ...
        for index, event in enumerate(final_events, 1):
          ...
          print(
              f"   [{index}/{total_events}] [ID: {event.get('id')}]"
              f" {event.get('teamAName')} vs {event.get('teamBName')}"
          )
          ...
          if link_file_path and (
            ...
            print(
                f"   [{index}/{total_events}] লিঙ্ক ডিকোর্ড হচ্ছে ➔"
                f" {event.get('teamAName')} vs {event.get('teamBName')}"
            )
        ...
        if RECEIVER_URL_1 and HOSTING_AUTH_TOKEN_1:
          targets.append(
              ("সার্ভার ১", RECEIVER_URL_1, HOSTING_AUTH_TOKEN_1)
          )
        ...
        for name, url, token in targets:
          ...
          try:
            ...
            if upload_res.status_code == 200:
              ...
              print(
                  f"❌ {name} আপলোড ফেল করেছে। স্ট্যাটাস কোড:"
